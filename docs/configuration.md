...
FM_LOCK_UNAVAILABLE_REPORT_EVERY=100   # acquisition attempts per printed refusal when the filesystem itself cannot create the lock (full, read-only, or unwritable parent); every process still reports its first refusal, 0 silences the report, and a non-numeric value uses 100
FM_LOCK_LEGACY_STEAL_SCAN_MAX=8   # maximum <lock>.steal levels walked when clearing chains left on disk by the pre-fix recursive reclaim; a live or fresh level stops the walk, and a non-numeric value uses 8
